...
//         borderRadius={'10px'}
//       >
//         <Box bgcolor={'red'} width='400px'>hello</Box>
//         {work && (
...
import React, { useEffect, useState } from 'react';
import { useParams, useNavigate } from 'react-router-dom';
import { TextField, Button, Box, Grid2, Avatar, Typography } from '@mui/material';
import API from '../../api/api';
import { useTheme } from '@emotion/react';
import { tokens } from '../../theme';
import Header from '../../components/Header';
  ...
  useEffect(() => {
    const fetchWorkDetails = async () => {
      try {
        const response = await API.get(`/getwork/${id}`);
        setWork(response.data.work);
        console.log(response.data) // Assuming response.data contains the work object
        setLoading(false);
      } catch (err) {
        setError('Failed to fetch work details');
        setLoading(false);
  ...
  return (
    <Box display={'flex'} flexDirection={'column'} height={'88%'} overflow={'auto'} margin={'10px'} p={0.1}>
      ...
      <Box
        ...
        borderRadius={'10px'}
      >
        <Box bgcolor={'red'} flex={1} display="flex" flexDirection="column" alignItems="center" p={2} borderRadius="10px">
          <Avatar src="/customer-image.jpg" sx={{ width: 80, height: 80, mb: 2 }} />
          <Typography variant="h6" color="white">{work.customer.customerName}</Typography>
          <Typography variant="body2" color="white">{work.customer.email}</Typography>
          <Typography variant="body2" color="white">{work.customer.mobileNo}</Typography>
          <Typography variant="body2" color="white">{work.customer.email}</Typography>
        </Box>
        <Box flex={2.5}>hi</Box>
        <Grid2 container spacing={2} gap={2} padding={"10px 20px"} color={colors.grey[200]}>
