...
const MutedWorksList = () => {
  ...
  const fetchEmployeesAndWorks = async () => {
    ...
    try {
      const [worksResponse, employeesResponse] = await Promise.all([
        API.get('/muted-works', { headers: { Authorization: `Bearer ${token}` } }),
  ...
  const handleDeleteWork = async () => {
    ...
    try {
      ...
      setWorks((prevWorks) => prevWorks.filter((work) => work._id !== selectedWorkId));
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: ({ row }) => (
        ...
        >
         <IconButton aria-label="delete-work" onClick={() => handleOpenDeleteModal(row._id)}>
            <Delete />
          </IconButton>
        </Box>
      ),
     ...
     <Modal open={deleteModalOpen} onClose={handleCloseDeleteModal}>
        ...
        >
          ...
          <Box display="flex" justifyContent="flex-end" gap={2}>
            ...
            <Button color="error" variant="contained" onClick={handleDeleteWork}>
              Delete
            </Button>
          </Box>
        </Box>
