...
const CustomerList = () => {
  ...
  const handleFileChange = (e, fileType) => {
    ...
    if ((fileType === 'aadharCard' || fileType === 'panCard') && !validDocTypes.includes(file.type)) {
      setError(`${fileType === 'aadharCard' ? 'Aadhar' : 'PAN'} card must be JPG, PNG, or PDF`);
      setSnackbarOpen(true);
      return;
    }
  ...
  const handleUploadFiles = async () => {
    ...
    try {
      ...
      if (files.aadharCard) {
        ...
        uploadPromises.push(
          API.post(`/customer-documents/upload/${selectedCustomer._id}/aadhar`, formData, {
            headers: {
      ...
      if (files.panCard) {
        ...
        uploadPromises.push(
          API.post(`/customer-documents/upload/${selectedCustomer._id}/pan`, formData, {
            headers: {
              Authorization: `Bearer ${token}`,
              'Content-Type': 'multipart/form-data',
            },
  ...
  const columns = [
    ...
    { field: "contactPerson", headerName: "Contact Person", flex: 1 },
    {
      ...
      align: "center",
      renderCell: ({ row }) => (
        <Box display={'flex'} justifyContent={'space-evenly'} alignItems={'center'} sx={{ height: '100%', width: '100%' }}>
          <Tooltip title="View">
  ...
  return (
    <Box display="flex" sx={{
          ...
          {selectedCustomer && (
            <>
              ...
              <Box sx={{
                ...
                <Typography variant="h6" sx={{ mb: 2, color: colors.grey[100], fontWeight: 'bold' }}>
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5, mb: 3 }}>
                  {[
                    ...
                    { label: 'WhatsApp No', value: selectedCustomer.whatsappNo },
                    { label: 'Email', value: selectedCustomer.email },
                    { label: 'PAN', value: selectedCustomer.PAN },
                    { label: 'Aadhaar No', value: selectedCustomer.AadharNo },
                    { label: 'Address', value: selectedCustomer.address, multiline: true },
                    { label: 'Company/Firm Name', value: selectedCustomer.companyName },
                    { label: 'Billing Name', value: selectedCustomer.billingName },
                    { label: 'Contact Person Name', value: selectedCustomer.contactPersonName },
                    { label: 'Contact Person Phone', value: selectedCustomer.contactPersonPhone },
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mb: 3 }}>
                  {[
                    ...
                    >
                      <Typography
                        variant="body2"
                        sx={{ width: 180, fontWeight: 'bold', color: colors.grey[200], flexShrink: 0 }}
                      >
                        {label}
                      </Typography>
                      {doc ? (
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, flex: 1 }}>
                          ...
                          <Box sx={{ display: 'flex', gap: 1 }}>
                            <Button
                              ...
                              onClick={() => handleViewFile(doc.url, doc.filename)}
                              sx={{
                                color: colors.grey[100],
                                borderColor: colors.grey[500],
                                '&:hover': { borderColor: colors.teal[300], bgcolor: colors.teal[900] },
                              }}
                            >
                              View
                            </Button>
                ...
                <Box sx={{
                  ...
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                    {[
                      { id: 'aadharCard', label: 'Upload Aadhar Card', accept: 'image/jpeg,image/png,application/pdf' },
                      { id: 'panCard', label: 'Upload PAN Card', accept: 'image/jpeg,image/png,application/pdf' },
                    ].map(({ id, label, accept }) => (
                      <Box key={id} sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                        ...
                        {files[id] && (
                          <Typography
                            variant="caption"
                            sx={{ color: colors.grey[300], maxWidth: 200, overflow: 'hidden', textOverflow: 'ellipsis' }}
                          >
                            {files[id].name}
                          </Typography>
                        )}
                    ...
                    <Button
                      ...
                      disabled={isUploading || (!files.aadharCard && !files.panCard)}
                      sx={{
                        bgcolor: colors.teal[300],
      ...
      <Modal
        ...
        sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}
      >
        <Box sx={{ padding: 2, width: "600px", height: '500px', overflow: 'auto', margin: 'auto', bgcolor: colors.bgc[100], borderRadius: 2 }}>
